...
class ViNT_Dataset_Calibration(Dataset):
    def __init__(
        self,
        data_folder: str,
        data_split_folder: str,
    ...
    def _build_index(self, use_tqdm: bool = False):
        ...
        for traj_name in tqdm.tqdm(self.traj_names, disable=not use_tqdm, dynamic_ncols=True):
            ...
            end_time = traj_len - self.end_slack - self.len_traj_pred * self.waypoint_spacing
            for curr_time in range(begin_time, end_time):
                ################ force the goal distance to be fixed ##################
                if traj_len - curr_time - 2 < self.max_dist_cat * self.waypoint_spacing:
                    break
                #######################################################################
                max_goal_distance = self.max_dist_cat * self.waypoint_spacing
                samples_index.append((traj_name, curr_time, max_goal_distance))
    ...
    def _load_index(self) -> None:
        ...
        index_to_data_path = os.path.join(
            self.data_split_folder,
            f"dataset_dist_{self.max_dist_cat}_context_{self.context_type}_n{self.context_size}_slack_{self.end_slack}_calibration.pkl",
        )
        try:
            # load the index_to_data if it already exists (to save time)
            with open(index_to_data_path, "rb") as f:
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        obs_traj_list = []
        for time in range(curr_time, goal_time - 1, self.waypoint_spacing):
            # Load images
            context = []
            if self.context_type == "temporal":
                # sample the last self.context_size times from interval [0, curr_time)
                context_times = list(
                    range(
                        time + -self.context_size * self.waypoint_spacing,
                        time + 1,
                        self.waypoint_spacing,
                    )
                )
            ...
            obs_traj_list.append(obs_image)
